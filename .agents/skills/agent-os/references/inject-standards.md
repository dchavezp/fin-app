...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `docs/standards/` (not in a subfolder).
...
Read `docs/standards/index.yml` to get the list of available standards and their descriptions.
...
@docs/standards/api/error-handling.md
@docs/standards/global/naming.md
...
1. **Detect Scenario** — Same as auto-suggest mode
2. **Parse Arguments:**
   - **Folder name** — `api` → inject all `.md` files in `docs/standards/api/`
   - **Folder/file** — `api/response-format` → inject `docs/standards/api/response-format.md`
   - **Root folder** — `root` → inject all `.md` files directly in `docs/standards/` (not in subfolders)
   - **Root file** — `root/naming` → inject `docs/standards/naming.md`
3. **Validate** — Check that specified files/folders exist. If not, show available standards and ask if they meant one of those.
4. **Inject Based on Scenario** — Same formatting as auto-suggest mode.
