...
Gather context and structure planning for significant work. **Must be run in plan mode.**
...
Check if `docs/product/` exists and contains files.

If it exists, read key files (like `mission.md`, `roadmap.md`, `tech-stack.md`) and use AskUserQuestion:

```
I found product context in docs/product/. Should this feature align with any specific product goals or constraints?
...
Read `docs/standards/index.yml` to identify relevant standards based on the feature being built.
...
**Note:** If `docs/specs/` doesn't exist, create it when saving the spec folder.
...
Create `docs/specs/{folder-name}/` with:
...
```
docs/specs/{YYYY-MM-DD-HHMM-feature-slug}/
